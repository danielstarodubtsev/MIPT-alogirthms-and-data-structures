...
void Solve() {
  ...
  for (int i = 0; i < edge_cnt; ++i) {
    ...
    edges[{from - 1, to - 1}] = cap;
    if (edges.find({to - 1, from - 1}) == edges.end()) {
      edges[{to - 1, from - 1}] = 0;
    }
  }
  ...
  while (true) {
    ...
    int goal = vertex_cnt - 1;
    bool found = Dfs(0, goal, used, graph, next, edges);
    ...
    while (cur_v != goal) {
      min_found = std::min(min_found, edges[{cur_v, next[cur_v]}]);
      cur_v = next[cur_v];
    }
    ...
    while (cur_v != goal) {
      edges[{cur_v, next[cur_v]}] -= min_found;
      edges[{next[cur_v], cur_v}] += min_found;
      cur_v = next[cur_v];
    }
  }
