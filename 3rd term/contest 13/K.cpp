...
signed main() {
  ...
  std::vector<Point> sum = MinkowskiSum(first, second);

  long double ans = cInf;
  for (size_t i = 0; i < sum.size(); ++i) {
    Segment cur = {sum[i], sum[(i + 1) % sum.size()]};
    ans = std::min(ans, PointSegmentDistance(cur.start, cur.end, Point(0, 0)));
  }

  std::cout << std::max(ans - cConst, (long double)0);
}