...
signed main() {
  ...
  std::vector<Point> intersections =
      SegmentCircleIntersection(Segment(first, second), circle);

  if (intersections.size() <= 1) {
    std::cout << Distance(first, second);
  } else {
    std::pair<Point, Point> first_pair = TangentPoints(first, circle);
    std::pair<Point, Point> second_pair = TangentPoints(second, circle);
    long double ans = cInf;
    std::vector<Point> first_vector = {first_pair.first, first_pair.second};
    std::vector<Point> second_vector = {second_pair.first, second_pair.second};
    for (Point x : first_vector) {
      for (Point y : second_vector) {
        long double angle = std::abs(
            std::acos(DotProduct(x - circle.Center(), y - circle.Center()) /
                      (circle.r * circle.r)));
        if (Equal(angle, 0) && x != y) {
          angle = cPi;
        }
        ans = std::min(
            ans, Distance(first, x) + Distance(second, y) +
                     circle.Perimeter() * (angle) / ((long double)2 * cPi));
      }
    }
